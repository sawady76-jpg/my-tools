...
def main():
    ...
    # ★変更点: 特定のファイルを読み込むように変更
    target_file = "発着信履歴_統合版.xlsx"
    if not os.path.exists(target_file):
        print(f"【エラー】対象ファイルが見つかりません: {target_file}")
        return

    print(f"  -> 読み込み: {target_file}")
    df_int = smart_read_excel(target_file, '内線通話')
    df_ext = smart_read_excel(target_file, '外線着信')
    # 時短勤務は存在しない可能性があるので空のDF
    df_short = pd.DataFrame()
    # もしあれば読み込む (シート名がわからないので一旦スキップまたはエラーハンドリング)
    try:
        # シート名一覧を取得して確認
        xl = pd.ExcelFile(target_file)
        if any('時短' in s for s in xl.sheet_names):
             # 簡易的に最初のマッチするシートを読む
             sheet = [s for s in xl.sheet_names if '時短' in s][0]
             df_short = smart_read_excel(target_file, sheet)
    except:
        pass
    ...
    for col in s6.columns:
        s6.loc['合計', col] = s6.loc[s6.index != '合計', col].sum()
    # 応答率を再計算（ゼロ除算防止）
    int_in = s6.loc['合計', ('内線','入電')] if ('内線','入電') in s6.columns else 0
    int_an = s6.loc['合計', ('内線','着電')] if ('内線','着電') in s6.columns else 0
    ext_in = s6.loc['合計', ('外線','入電')] if ('外線','入電') in s6.columns else 0
    ext_an = s6.loc['合計', ('外線','着電')] if ('外線','着電') in s6.columns else 0
    s6.loc['合計', ('内線','応答率')] = int_an / int_in if int_in > 0 else 0
    s6.loc['合計', ('外線','応答率')] = ext_an / ext_in if ext_in > 0 else 0
