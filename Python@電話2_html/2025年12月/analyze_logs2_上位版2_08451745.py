...
def decorate_excel(filename):
    ...
    def set_cell_style(cell, style_type="data", number=None):
        ...
        if number is not None:
            if isinstance(number, (int, float)):
                # 率の判定
                if "率" in str(cell.value) or "％" in str(cell.value):
    ...
    for sheet_name in wb.sheetnames:
        ...
        if sheet_name == '1.着信件数':
            # --- 余分な行（行3の「拠点」インデックス名）を削除 ---
            if ws.cell(3, 1).value == '拠点' and ws.cell(4, 1).value not in [None, '拠点']:
                ws.delete_rows(3, 1)
                max_r = ws.max_row  # 行削除後に再計算
            ...
            for r in range(3, max_r + 1):
                style = "total" if r == max_r else "data"
                for c in range(1, 8):
            ...
            # --- 余分な行（行7の「拠点」インデックス名）を削除 ---
            if ws.cell(7, 1).value == '拠点' and ws.cell(8, 1).value not in [None, '拠点']:
                ws.delete_rows(7, 1)
            ...
            ws.merge_cells('B4:D4') # 内線
            ws.merge_cells('E4:G4') # 外線
