...
def decorate_excel(filename):
    ...
    for sheet_name in wb.sheetnames:
        ...
        if sheet_name == '1.着信件数':
            ...
            # 【修正点1】右側の表(J列以降)が長くても、左側の表(A列)の罫線が伸びないようにする
            # A列に値が入っている行だけを「メイン表の最終行」とみなす
            real_max_r_main = 0
            for r in range(3, max_r + 20): # 少し余裕を見て探査
                if ws.cell(row=r, column=1).value is None and ws.cell(row=r, column=2).value is None:
                    real_max_r_main = r - 1
                    break
            
            if real_max_r_main < 3: real_max_r_main = max_r # 見つからない場合の保険

            for r in range(3, real_max_r_main + 1):
            ...
            # ミニ集計 (Row 13-15)
            for c in range(11, 14): # K-M
                set_cell_style(ws.cell(row=13, column=c), "sub_header")
            
            ws['J13'].fill = white_fill
            ws['J13'].border = border_all
            ...
            ws['A5'] = "拠点"
            ws.merge_cells('A5:A6') # 修正
            ws.merge_cells('B5:D5') # 内線 修正
            ws.merge_cells('E5:G5') # 外線 修正
            ...
            # --- 上部集計 (M2:Q3) ---
            # 1行挿入されたので、M2にあったヘッダはM3に移動しているはずです。
            # ただし、元々 to_excel の startrow=0 で出力しているため、
            # Insert前: Row1(Header), Row2(Data)
            # Insert後: Row2(Header), Row3(Data)
            ...
            # --- グループ集計 (M6起点) ---
            # 元々 startrow=4 (Excel 5行目)
            # Insert後: Excel 6行目 スタート
