...
def main():
    ...
    s1['内線_入電'] = s1.index.map(get_counts(df_int, 'target_base')).fillna(0).astype(int)
    s1['内線_着電'] = s1.index.map(get_counts(df_int_valid, 'final_base')).fillna(0).astype(int)
    s1['外線_入電'] = s1.index.map(get_counts(df_ext, 'target_base')).fillna(0).astype(int)
    s1['外線_着電'] = s1.index.map(get_counts(df_ext_valid, 'final_base')).fillna(0).astype(int)
    s1['他拠点へ転送'] = s1.index.map(TRANS_TO).fillna('')
    s1['他拠点から転送'] = s1.index.map(TRANS_FROM).fillna('')

    # --- Sheet 2 ---
    cols = ['final_name', 'final_base', '通話時間', 'day']
    s2 = pd.DataFrame()

    if (not df_int_valid.empty) or (not df_ext_valid.empty):
        # 個別に集計してから結合する安全な方式に変更
        ...
        if not df_int_valid.empty and set(cols).issubset(df_int_valid.columns):
            ...
            s2_int = pd.DataFrame(columns=['内線件数', '内線合計'])
        ...
        if not df_ext_valid.empty and set(cols).issubset(df_ext_valid.columns):
            ...
            s2_ext = pd.DataFrame(columns=['外線件数', '外線合計'])

        # 結合 (Outer Join)
        s2 = s2_int.join(s2_ext, how='outer').fillna(0)
        
        # 平均計算
        s2['内線平均時間'] = (s2['内線合計'] / s2['内線件数'].replace(0, 1)).apply(my_round)
        s2['外線平均時間'] = (s2['外線合計'] / s2['外線件数'].replace(0, 1)).apply(my_round)
        
        # 日別集計 (concatして一気にやる)
        df_all_valid = pd.concat([
            df_int_valid[cols] if not df_int_valid.empty else pd.DataFrame(),
            df_ext_valid[cols] if not df_ext_valid.empty else pd.DataFrame()
        ])
        ...
        if not df_all_valid.empty:
            ...
            out_cols = ['名前', '拠点', '内線件数', '内線平均時間', '外線件数', '外線平均時間'] + list(range(1, 32)) + ['稼働日', '内外線計']
            # 存在しない列があれば0埋め
            for c in out_cols:
                if c not in s2.columns: s2[c] = 0
            s2 = s2[out_cols]

    # --- Sheet 3 ---
    s3 = s1[['外線_着電']].reset_index().copy()
    s3.columns = ['拠点名', '25年2月から外線のみ']
    if not s2.empty:
        personnel = s2.groupby('拠点')['名前'].nunique()
        s3['人員'] = s3['拠点名'].map(personnel).fillna(0).astype(int)
    else: s3['人員'] = 0
    s3['1人当たり／月'] = s3.apply(lambda r: my_round(r['25年2月から外線のみ'] / r['人員']) if r['人員'] > 0 else 0.0, axis=1)
    total_s3 = s3['25年2月から外線のみ'].sum()
    s3['全体からの比率'] = s3['25年2月から外線のみ'].apply(lambda x: f"{my_round(x / total_s3 * 100)}%" if total_s3 > 0 else "0.0%")
    ...
    if not df_ext_valid.empty and 'time' in df_ext_valid.columns:
        df_ext_valid = df_ext_valid.dropna(subset=['time'])
        df_ext_biz = df_ext_valid[
            df_ext_valid['time'].apply(lambda t: BIZ_START <= t <= BIZ_END)
        ]
        s5_counts = df_ext_biz['final_base'].value_counts()
        s5['営業時間内_外線のみ'] = s5.index.map(s5_counts).fillna(0).astype(int)
    else:
        s5['営業時間内_外線のみ'] = 0
    ...
    try:
        with pd.ExcelWriter(output_filename) as writer:
            s1.reset_index().to_excel(writer, sheet_name='1.着信件数', index=False)
            s2.to_excel(writer, sheet_name='2.従業員別', index=False)
            s3.to_excel(writer, sheet_name='3.関数_拠点別', index=False)
            if not df_short.empty:
                df_short.to_excel(writer, sheet_name='4.時短勤務', index=False)
            else:
                pd.DataFrame({'info': ['データなし']}).to_excel(writer, sheet_name='4.時短勤務', index=False)
